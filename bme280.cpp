...
void format_output(struct measure *mm, char *buf)
{
    ...
    {
        sprintf(buf, "%s Temp: %2.2f\tHumidity: %2.2f\n",buf, mm->bme.tempC, mm->bme.humid);
        return;
    }
    ...
    {
        // BME results
        if (*p == 'T') sprintf(buf, "%s Temp: %2.2f",buf,mm->bme.tempC);
        else if (*p == 'H') sprintf(buf, "%s Humidity: %2.2f",buf,mm->bme.humid);
        else if (*p == 'P') sprintf(buf, "%s Pressure: %2.2f",buf,mm->bme.pressure/100);
        else if (*p == 'M') sprintf(buf, "%s Height: %2.2f",buf,mm->bme.height);
